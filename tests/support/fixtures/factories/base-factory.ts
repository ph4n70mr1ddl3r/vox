...
export abstract class BaseFactory {
    ...
    return withRetry(async () => {
      ...
      if (!response.ok()) {
        ...
        const error = new Error(
          `${context} failed: ${method} ${url} - ${response.status()} ${errorText}`
        )
   ...
   * @param resourceType - API resource type (e.g., 'users', 'campaigns')
   * @param displayName - Display name for logging (defaults to resourceType)
   * @param timeoutMs - Timeout per deletion operation (default 30000ms)
   * @returns Cleanup result with success/failure counts and errors
   */
  async cleanup(
    resourceType: string,
    displayName?: string,
    timeoutMs: number = 30000
  ): Promise<CleanupResult<string>> {
