...
export class TrustConnectionFactory extends BaseFactory {
  ...
  async createConnection(options: CreateConnectionOptions): Promise<TrustConnection> {
    ...
    try {
      ...
      if (!response.ok()) {
        ...
        throw new Error(
          `Failed to create trust connection from ${options.fromUserId} to ${options.toUserId}: ${response.status()} ${errorText}`
        )
      }
      ...
      if (error instanceof Error) {
        console.error(
          `TrustConnectionFactory.createConnection failed for ${options.fromUserId} -> ${options.toUserId}:`,
          error.message
        )
      }
      throw error
  ...
  async acceptConnection(connectionId: string): Promise<TrustConnection> {
    try {
      const response = await this.request.patch(
        `${this.baseURL}/trust-connections/${connectionId}/accept`
      )

      if (!response.ok()) {
        const errorText = await response.text()
        throw new Error(
          `Failed to accept connection ${connectionId}: ${response.status()} ${errorText}`
        )
      }
      ...
      if (error instanceof Error) {
        console.error(
          `TrustConnectionFactory.acceptConnection failed for ${connectionId}:`,
          error.message
        )
      }
      throw error
  ...
  async rejectConnection(connectionId: string): Promise<TrustConnection> {
    try {
      const response = await this.request.patch(
        `${this.baseURL}/trust-connections/${connectionId}/reject`
      )

      if (!response.ok()) {
        const errorText = await response.text()
        throw new Error(
          `Failed to reject connection ${connectionId}: ${response.status()} ${errorText}`
        )
      }
      ...
      if (error instanceof Error) {
        console.error(
          `TrustConnectionFactory.rejectConnection failed for ${connectionId}:`,
          error.message
        )
      }
      throw error
  ...
  async createNetwork(
    ...
    if (errors.length > 0) {
      console.warn(
        `createNetwork: ${acceptedConnections.length} succeeded, ${errors.length} failed`
      )
