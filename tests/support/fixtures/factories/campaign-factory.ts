...
import {
  ...
  CAMPAIGN_CATEGORIES,
  CAMPAIGN_STATUSES,
  CAMPAIGN_DEFAULT_DURATION_DAYS,
} from '../constants'
...
export class CampaignFactory extends BaseFactory {
  ...
  async createCampaign(options: CreateCampaignOptions): Promise<Campaign> {
    ...
    const campaignData = {
      ...
      endDate:
        options.endDate ||
        new Date(Date.now() + CAMPAIGN_DEFAULT_DURATION_DAYS * 24 * 60 * 60 * 1000),
    }
    ...
    try {
      ...
      if (!response.ok()) {
        ...
        throw new Error(
          `Failed to create campaign for brand ${options.brandId}: ${response.status()} ${errorText}`
        )
      }
      ...
      if (error instanceof Error) {
        console.error(
          `CampaignFactory.createCampaign failed for brand ${options.brandId}:`,
          error.message
        )
      }
      throw error
  ...
  async updateStatus(campaignId: string, status: Campaign['status']): Promise<Campaign> {
    try {
      ...
      if (!response.ok()) {
        const errorText = await response.text()
        throw new Error(
          `Failed to update campaign status for ${campaignId} to ${status}: ${response.status()} ${errorText}`
        )
      }
